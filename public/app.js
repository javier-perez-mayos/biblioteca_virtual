...
function initializeEventListeners() {
  ...
  document.getElementById('editBookCoverInput')?.addEventListener('change', handleEditBookCoverUpload);
...
function updateUIForAuth(isAuthenticated) {
  ...
  if (isAuthenticated && currentUser) {
    ...
    if (adminPanelLink) {
      adminPanelLink.style.display = currentUser.is_admin ? 'block' : 'none';
    }
  } else {
...
function createBookCard(book) {
  ...
  card.onclick = (e) => {
    // Don't open detail if clicking edit button
  ...
  // Show edit button if user owns the book
  const isOwner = currentUser && book.added_by_user_id === currentUser.id;
  const editButton = isOwner ?
    `<button class="btn-icon btn-secondary book-edit-btn" onclick="event.stopPropagation(); openEditBookModal(${book.id});" title="${t('editBook')}">
      ✏️
    </button>` : '';
...
async function startBarcodeScanner() {
  ...
  if (typeof Html5Qrcode === 'undefined') {
    ...
    alert(t('errorOccurred') + ': Barcode scanner library not loaded. Please refresh the page.');
    stopBarcodeScanner();
    return;
  }
  ...
  try {
    ...
    if (!scannerElement) {
      throw new Error('Scanner div element not found');
    }
    ...
    try {
      ...
      if (devices && devices.length) {
        ...
        // If no cameras found, add fallback option
        if (devices.length === 0) {
          cameraSelect.innerHTML = '<option value="environment" selected>Default (Back Camera)</option>';
        } else {
          // Add each camera as an option
          devices.forEach((device, index) => {
            const option = document.createElement('option');
            option.value = device.id;
            option.text = device.label || `Camera ${device.id.substr(0, 8)}...`;
          ...
          console.log('Camera dropdown populated with device IDs');
        }
      } else {
...
function onBarcodeScanned(decodedText) {
  ...
  // Show detected ISBN
  document.getElementById('detectedIsbn').textContent = decodedText;
...
function showManualSearchFallback(bookData, fallbackDiv) {
  ...
  fallbackDiv.innerHTML = `
    <div style="font-size: 3rem; margin-bottom: 1rem;">📚</div>
    <p style="margin-bottom: 1rem; color: #666;">
      <strong>${t('noCoverAvailable') || 'No cover image available'}</strong>
    </p>
    <p style="margin-bottom: 1rem; font-size: 0.9rem; color: #888;">
      ${t('searchCoverOnline') || 'You can search for the book cover online:'}
    </p>
    <a href="${googleImagesUrl}"
       target="_blank"
       style="display: inline-block; padding: 0.75rem 1.5rem; background: #4285f4; color: white; text-decoration: none; border-radius: 6px; font-weight: 500; transition: background 0.2s;"
       onmouseover="this.style.background='#357ae8'"
       onmouseout="this.style.background='#4285f4'">
      🔍 ${t('searchGoogleImages') || 'Search on Google Images'}
    </a>
